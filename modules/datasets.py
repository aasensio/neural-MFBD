...
class DatasetSST(torch.utils.data.Dataset):
    ...
    def __init__(self, config):
        ...
        print(f"Reading SST file : {config['training_file_sst']}")
        ...
        self.file = zarr.open(config['training_file_sst'], 'r')
    ...
    def __getitem__(self, index):
        ...
        # print('SST : ', out_images.shape, out_images_apodized.shape, out_wl.shape, sigma.shape, weight.shape)
...
class DatasetHiFi(torch.utils.data.Dataset):
    ...
    def __init__(self, config):
        ...
        print(f"Reading HiFi {config['training_file_hifi']}")
        ...
        # Open file and read all focused/defocused images, together with the modes
        self.file = zarr.open(config['training_file_hifi'], 'r')
    ...
    def __getitem__(self, index):
        ...
        # Compute contrast and change the weight for the objects with low (or very high) contrast
        contrast = 100.0 * np.std(out_bb) / np.mean(out_bb)
        if (contrast > 25.0):
            weight *= 0.0
        if (contrast < 3.0):
            weight *= 0.05

        # print('Hifi : ', out_images.shape, out_images_apodized.shape, out_wl.shape, sigma.shape, weight.shape)
...
class Dataset_validation(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, config, n_pixel=None):
...
class DatasetSST_validation(Dataset_validation):
    ...
    """
    def __init__(self, config, iregion, index, wav=0, mod=0, cam=0, npix=None, step=40):
        super(DatasetSST_validation, self).__init__(config, npix)
...
class DatasetHiFi_validation(Dataset_validation):
    ...
    """
    def __init__(self, config, root, nac=12, npix=None, step=40):
        super(DatasetHiFi_validation, self).__init__(config, npix)
