    ...
    def __init__(self, config):
        ...
        for i in range(self.config['gradient_steps']):
            ...
            if (self.config['type_cleaning'] == 'conv1d'):
                nets.append(nn.Conv1d(self.config['n_modes'], self.n_channels, kernel_size=3, padding=1))
                nets.append(nn.GELU())
...
class Model(nn.Module):
    ...
    def update_modes(self, modes, residual, reconstructed_ft, weight, dpsfft_da, rho):
    ...
    def forward(self, frames, frames_apod, wl, sigma, weight, optimize=True, image_filter='gaussian'):
        ...
        for loop in range(self.config['gradient_steps']):
            ...
            modes, gradient = self.update_modes(modes, residual, reconstructed_ft, weight, dpsfft_da, 0.05 * self.denoise_modes_net.rho[loop])
