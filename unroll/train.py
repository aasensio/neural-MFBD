...
class FastMFBD(object):
    def __init__(self, configuration_file=None, checkpoint=None):
        ...
        if (self.config['dataset_instrument'] == 'All'):
            ...
            self.dataset = torch.utils.data.ConcatDataset([self.dataset_sst, self.dataset_hifi])
            # Get number of pixels and frames from the dataset                
        ...
        self.train_index = idx[0:int((1-self.config['validation_split'])*n_training)]
        self.validation_index = idx[int((1-self.config['validation_split'])*n_training):]
