...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config, iregion, index, wav=0, mod=0, cam=0):
        ...
        self.wb, self.nb, self.wl = read_images(iregion, index, wav, mod, cam)
...
class FastMFBD(object):
    def __init__(self, checkpoint, gpu=2):
        """
        ...
        self.model.config['gradient_steps'] = 150

        for param in self.model.parameters():
            param.requires_grad = False
                
    def validate_fov(self, outf, iregion, index, wav=0, mod=0, cam=0):
        """
        Validate for one epoch
        """

        # Read training and validation sets
        self.dataset = Dataset(self.config, iregion, index, wav=wav, mod=mod, cam=cam)
        ...
        for batch_idx, (frames, frames_apod, wl, xy, minval, maxval, sigma, weight) in enumerate(t):
            ...
            with torch.no_grad():
                reconstructed, modes = self.model.classic_reconstructed(frames, frames_apod, wl, sigma, weight, image_filter=filter)
...
if (__name__ == '__main__'):
    ...
    checkpoint = 'weights/2023-01-13-17:06_ep_30.pth'

    deep_mfbd_network = FastMFBD(checkpoint=checkpoint, gpu=2)

    # for i in range(20, 40):
        # deep_mfbd_network.validate_fov(f'reconstructed/validation_spot_8542_{i:02d}.h5', iregion=7, index=i, wav=7)

    for i in range(2):
        deep_mfbd_network.validate_fov(f'reconstructed/validation_plage_8542_{i:02d}_classic.h5', iregion=4, index=i, wav=7)

    # for i in range(300, 320):
        # deep_mfbd_network.validate_fov(f'reconstructed/validation_spot_3934_{i:02d}_classic.h5', iregion=5, index=i, wav=13)