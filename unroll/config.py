...
class Config(object):
    ...
    def __init__(self, configuration_file=None, training=True):
        ...
        if (training):
            ...
            if 'training set sst' in self.config_dict['dataset']:
                self.hyperparameters['training_file_sst'] = self.config_dict['dataset']['training set sst']
            if 'training set hifi' in self.config_dict['dataset']:
                self.hyperparameters['training_file_hifi'] = self.config_dict['dataset']['training set hifi']
            self.hyperparameters['validation_split'] = float(self.config_dict['dataset']['validation split'])                                    

            if (self.hyperparameters['dataset_instrument'] == 'HiFi' or self.hyperparameters['dataset_instrument'] == 'All'):
