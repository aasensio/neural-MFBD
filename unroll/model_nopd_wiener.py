...
class CleanNetwork(nn.Module):
    def __init__(self, config, correction_mode='modes'):
        super().__init__()
        ...
        if (self.correction_mode == 'modes'):
            print('NN corrects modes')
            self.n_input = self.config['n_modes']

        if (self.correction_mode == 'gradient'):
            print('NN estimates modes from modes+gradient')
            self.n_input = 2 * self.config['n_modes']
        ...
        for i in range(self.config['gradient_steps']):
            ...
            if (self.config['type_cleaning'] == 'fc'):
                nets.append(nn.Linear(self.n_input, 128))
                nets.append(nn.ReLU())
            ...
            if (self.config['type_cleaning'] == 'conv1d'):
                nets.append(nn.Conv1d(self.n_input, self.n_channels, kernel_size=3, padding=1))
                nets.append(nn.GELU())
    ...
    def forward(self, x, index, gradient=None, rho=None):
        ...
        if (self.correction_mode == 'gradient'):
            ...
            if (self.config['type_cleaning'] == 'conv1d'):
                xt = torch.transpose(x, 1, 2)
                gradientt = torch.transpose(gradient, 1, 2)
                xt = torch.cat([xt, gradientt], dim=1)
                xt = self.net[index](xt)
                out = x + torch.transpose(xt, 1, 2)
...
class Model(nn.Module):
    def __init__(self, config, cutoff_mask=0.90):
        """
        ...
        self.update_net = CleanNetwork(self.config, correction_mode=self.correction_mode)
        ...
        for i in range(self.config['gradient_steps']):   
            ...
            pars = list(self.update_net.net[i].parameters()) + list([self.update_net.rho[i]])
        ...
        # 
    def change_pixel_size(self, diffraction=0.9, npix=64, npix_apodization=12, diffraction_limit=0.9):

        n_wavelengths = len(self.config['wavelengths'])
        print("Chaning number of pixels from the one used during training")

        self.config['n_pixel'] = npix
        self.config['npix_apodization'] = npix_apodization

        if not hasattr(diffraction_limit, "__len__"):
            diffraction_limit = [diffraction_limit] * n_wavelengths
        ...
        for loop in range(n_wavelengths):
            ...
            print(f"Defining diffraction limit for wavelength {wl} to {diffraction_limit[loop]} - npix to {npix:3d} - npix_apod to {npix_apodization:3d}")
            ...
            if (self.config['basis_for_wavefront'].lower() == 'zernike'):
                ...
                basis = np.zeros((self.config['n_modes'], npix, npix))
            ...
            if (self.config['basis_for_wavefront'].lower() == 'kl'):
                print(f"Computing KL modes for {wl}")
                kl = kl_modes.KL()
                basis = kl.precalculate(npix_image = npix, 
                                    n_modes_max = self.config['n_modes'], 
                                    first_noll = 2, 
                                    overfill=overfill)
                basis /= np.max(np.abs(basis), axis=(1, 2), keepdims=True)
            ...
            # Define the diffraction mask to tamper high frequencies for each wavelength
            cutoff = self.config['diameter'][loop] / (self.config['wavelengths'][loop] * 1e-8) / 206265.0
            freq = np.fft.fftfreq(npix, d=self.config['pix_size'][loop]) / cutoff
            
            xx, yy = np.meshgrid(freq, freq)
            rho = np.sqrt(xx ** 2 + yy ** 2)
            mask_diff = rho <= diffraction_limit[loop]          
            mask_diffraction_shift = np.fft.fftshift(mask_diff)

            if (int(wl) == 3934):
                self.mask_3934 = torch.tensor(mask_diffraction_shift.astype('float32')).to(self.pupil_8542.device)
                self.pupil_3934 = torch.tensor(pupil.astype('float32')).to(self.pupil_8542.device)
                self.basis_3934 = torch.tensor(basis.astype('float32')).to(self.pupil_8542.device)
            if (int(wl) == 6173):
                self.mask_6173 = torch.tensor(mask_diffraction_shift.astype('float32')).to(self.pupil_8542.device)
                self.pupil_6173 = torch.tensor(pupil.astype('float32')).to(self.pupil_8542.device)
                self.basis_6173 = torch.tensor(basis.astype('float32')).to(self.pupil_8542.device)
            if (int(wl) == 6563):
                self.mask_6563 = torch.tensor(mask_diffraction_shift.astype('float32')).to(self.pupil_8542.device)
                self.pupil_6563 = torch.tensor(pupil.astype('float32')).to(self.pupil_8542.device)
                self.basis_6563 = torch.tensor(basis.astype('float32')).to(self.pupil_8542.device)
            if (int(wl) == 8542):
                self.mask_8542 = torch.tensor(mask_diffraction_shift.astype('float32')).to(self.pupil_8542.device)
                self.pupil_8542 = torch.tensor(pupil.astype('float32')).to(self.pupil_8542.device)
                self.basis_8542 = torch.tensor(basis.astype('float32')).to(self.pupil_8542.device)
            
        self.window = torch.tensor(window.astype('float32')).to(self.pupil_8542.device)
    ...
    def image_from_modes(self, modes, frames_ft, frames_apod, sigma, weight, pupil, basis, mask, image_filter='gaussian', derivatives=True):

        # Compute PSFs from modes
        wavefront, psf, psf_ft, dpsfft_da = self.compute_psfs(modes, pupil, basis, derivatives=derivatives)
    ...
    def compute_gradient(self, modes, residual, reconstructed_ft, weight, dpsfft_da):
    ...
    def forward_analytical(self, frames, frames_apod, wl, sigma, weight, optimize=True, image_filter='gaussian'):
        ...
        for loop in range(self.config['gradient_steps']):
            ...
            gradient = self.compute_gradient(modes, residual, reconstructed_ft, weight, dpsfft_da)

            if (self.correction_mode == 'modes'):
                # Update modes using GD
                modes = modes - 0.05 * self.update_net.rho[loop] * gradient
                # Clean modes
                modes = self.update_net(modes, loop)

            if (self.correction_mode == 'gradient'):
                
                # Compute update using NN
                modes = self.update_net(modes, loop, gradient=gradient, rho=1.0 * self.update_net.rho[loop])
                
            # Compute image from modes
            loss, wavefront, psf, degraded, reconstructed_ft, residual, dpsfft_da = self.image_from_modes(modes, frames_ft, frames_apod, sigma, weight, pupil, basis, mask, image_filter='gaussian')
            
            loss = torch.mean(loss) / self.config['gradient_steps'] #+ 0.1 * torch.mean(torch.abs(modes))
            ...
            #print(loop, loss.item(), 0.1 * torch.mean(torch.abs(modes)).item(), total_loss.item()) 
        ...
        return modes, psf, wavefront, degraded, reconstructed, reconstructed_apod, total_loss, ratio_loss
    ...
    def forward(self, frames, frames_apod, wl, sigma, weight, optimize=True, image_filter='gaussian'):
        ...
        for loop in range(self.config['gradient_steps']):
            ...
            loss, wavefront, psf, degraded, reconstructed_ft, residual, _ = self.image_from_modes(modes, frames_ft, frames_apod, sigma, weight, pupil, basis, mask, image_filter='gaussian', derivatives=False)
            loss = torch.sum(loss) / self.config['gradient_steps']            
            gradient = torch.autograd.grad(loss, modes)[0]
            ...
            # Compute image from modes and loss function to optimize the networks
            loss, wavefront, psf, degraded, reconstructed_ft, residual, dpsfft_da = self.image_from_modes(modes, frames_ft, frames_apod, sigma, weight, pupil, basis, mask, image_filter='gaussian')
            ...
            if (optimize):
                ...
                modes = modes.clone().detach().requires_grad_(True)
                residual = residual.clone().detach()
                reconstructed_ft = reconstructed_ft.clone().detach()                
    ...
    def forward_reconstructed(self, frames, frames_apod, wl, sigma, weight, image_filter='gaussian'):
        ...
        # Predict the corrected image        
        modes, psf, wavefront, degraded, reconstructed, reconstructed_apod, loss, ratio_loss = self.forward(frames, frames_apod, wl, sigma, weight, optimize=False, image_filter=image_filter)
